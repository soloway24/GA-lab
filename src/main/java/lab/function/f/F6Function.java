...
public class F6Function implements FitnessFunction<Double, Double> {
    ...
    public F6Function(int arity, int chromosomeLength, double minX, double maxX, int argumentPrecision, double a, double m) {
    ...
    public Double getMaxFitness() {
        return evaluate(getOptimalIndividual(STANDARD).get());
    }
    ...
    public Double evaluate(Individual individual) {
        ...
        double sum = IntStream.range(0, n)
                .mapToDouble(i -> {
                    double xi = xs.get(i);
                    return sin(xi) * pow(sin((i + 1) * pow(xi, 2) / PI), 2 * m);
                })
                .sum();
