...
public class F2Function implements FitnessFunction<Double, Double> {
    ...
    public F2Function(int arity, int chromosomeLength, double minX, double maxX, int argumentPrecision, double a) {
...
//        return List.of(GRAY);
    ...
    public Double evaluate(Individual individual) {
        ...
        double sumX = xs.stream()
                .map(x -> 10 * cos(2 * PI * x) - pow(x, 2))
                .reduce(0.0, Double::sum);

        return n * abs(10 * cos(2 * PI * a) - pow(a, 2)) + sumX;
    }
