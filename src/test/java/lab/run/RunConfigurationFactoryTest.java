...
import static lab.selection.SelectorType.RWS;
...
class RunConfigurationFactoryTest {
    ...
    private final RunConfigurationFactory runConfigurationFactory = new RunConfigurationFactory();
    ...
    public void init() {
        ...
        when(function1.getUnsupportedSelectorTypes(OperatorType.NONE)).thenReturn(List.of(SUS));
        ...
        when(rwsSelector.getSelectorType()).thenReturn(RWS);
        when(susSelector.getSelectorType()).thenReturn(SUS);
        selectors = List.of(rwsSelector, susSelector);
    }
